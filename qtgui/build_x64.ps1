...
rsrc -manifest ebookdownloader.manifest -ico ebookdownloader.ico -arch amd64 -o rsrc_x64.syso

go build -ldflags "-H windowsgui -w -s main.Commit=$commit_id -X 'main.BuildTime=$build_time' -X main.Version=$last_tag -linkmode external -extldflags '-static'" -o ebookdl_gui.exe
