...
var _ edl.EBookDLInterface = Zhhbq{}

// Zhhbq https://www.zhhbq.com/ 小说网
type Zhhbq struct {
	URL  string
	Lock *sync.Mutex
}
...
func NewZhhbq() Zhhbq {
	return Zhhbq{
		URL:  "https://www.zhhbq.com/",
		Lock: new(sync.Mutex),
	}
}

// GetBookBriefInfo 获取小说的信息
func (this Zhhbq) GetBookBriefInfo(bookid string, proxy string) edl.BookInfo {
	var bi edl.BookInfo
...
// GetBookInfo 获取小说的信息
func (this Zhhbq) GetBookInfo(ctx context.Context, bookid string, proxy string) edl.BookInfo {
	...
	if proxy != "" {
		...
		//i := 12，因为最前面的12章是：显示最新章节信息，需要忽略掉
		for i := 12; i < len(ddNode); i++ {
			var tmp edl.Chapter
...
func (this Zhhbq) DownloadChapters(Bi edl.BookInfo, proxy string) edl.BookInfo {
	result := Bi //先进行赋值，把数据
	var chapters []edl.Chapter
...
func (this Zhhbq) DownloaderChapter(ResultChan chan chan edl.Chapter, pc edl.ProxyChapter, wg *sync.WaitGroup) {
	wg.Add(1)
	defer wg.Done()
	...
	go func(pc edl.ProxyChapter) {
		...
		if proxy != "" {
			...
			contentNode, _ := htmlquery.FindOne(doc, "//div[@class='content']/div[@id='content']")
